...
- (void)drawScanRect
{
    //int XRetangleLeft = _viewStyle.xScanRetangleOffset;//666666//x1
    int XRetangleLeft = 0;//x1
    
    //CGSize sizeRetangle = CGSizeMake(self.frame.size.width - XRetangleLeft*2, self.frame.size.width - XRetangleLeft*2);//666666
    CGSize sizeRetangle = self.frame.size;
    ...
    //扫码区域Y轴最小坐标
    //CGFloat YMinRetangle = self.frame.size.height / 2.0 - sizeRetangle.height/2.0 - _viewStyle.centerUpOffset;//666666//y1
    CGFloat YMinRetangle = 0;//y1
    //CGFloat YMaxRetangle = YMinRetangle + sizeRetangle.height;//666666//y2
    CGFloat YMaxRetangle = sizeRetangle.height;//y2
    //CGFloat XRetangleRight = self.frame.size.width - XRetangleLeft / 2;//666666//x2
    CGFloat XRetangleRight = self.frame.size.width;//x2
    
    
    NSLog(@"frame:%@",NSStringFromCGRect(self.frame));//frame:{{50, 174}, {220, 220}}
...
+ (CGRect)getScanRectWithPreView:(UIView*)view style:(LBXScanViewStyle*)style
{
    ...
    CGSize sizeRetangle = view.bounds.size;
